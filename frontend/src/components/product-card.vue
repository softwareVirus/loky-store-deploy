...
div(:class="(isOrder ? 'product-shopping-card' : 'product-card') + ' ' + (!product.inStock ? 'disabled-gray' : '')")
  div(:class="isOrder ? 'product-header product-shopping-header' : 'product-header'")
    ...
    img.product-image(:src="'/src/assets/Untitled.png'.replace('@','/src')" v-if="!isOrder")
  .product-card-info
...
export default {
  ...
  setup(props) {
    ...
    const isFilled = ref(
      store.state.user !== null ? store.state.user.favoriteProducts.includes(_id.value) : false
    )
    ...
    const deleteValue = () => {
      console.log(
        props.refValue.length === 1
          ? []
          : props.refValue
              .slice(0, props.index)
              .concat(props.refValue.slice(props.index + 1, props.refValue.length))
      )
      props.updateList(
        props.refValue.length === 1
          ? []
