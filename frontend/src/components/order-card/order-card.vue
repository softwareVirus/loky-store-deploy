...
export default {
  ...
  async setup(props) {
    ...
    const isFavorite = ref(
      store.state.user !== null ? store.state.user.favoriteProducts.includes(routeId) : false
    )
    ...
    window.onbeforeunload = () =>
      handleBeforeUnmountOrUnload(quantity.value, selectedDimension.value, selectedSize.value)
    ...
    const handleAddToCart = () => {
      ...
      console.log(list)
      list = list !== null ? list : []
      let isIncludeProduct = list
        .map((x) => x.product._id + ' ' + x.dimension + x.size)
        .indexOf(routeId + ' ' + selectedDimension.value + selectedSize.value)
      ...
      localStorage.setItem(
        'productList:' + new String(store.state.user._id),
        JSON.stringify(
          isIncludeProduct !== -1
            ? list
            : [
