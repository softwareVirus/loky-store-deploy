...
export default {
  ...
  async setup(props) {
    ...
    const orderDataWithId = computed(() =>
      store.state.orders.map((item) => ({
    ...
    const tableContent = ref(
      store.state.orders.map((item) => ({
        ...
        orderDate: format(new Date(item.orderDate), 'DD-MM-YYYY HH:mm:ss'),
    ...
    const data = computed(() => {
      ...
      return popupType.value === 'product'
        ? orderDataWithId.value[selectedIndex.value].idOrderCart
