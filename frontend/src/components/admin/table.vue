      ...
      tr(v-for="(item,index) in sortedItems" :key="item._id")
        td.table-row(v-for="(column,index) in Object.keys(item).filter(x => x.indexOf('_id') !== 0 || (x.indexOf('_id') == 0 && x[3] != undefined && x[3].toLowerCase() == x[3]))" :key="'12x'+item._id")
          ...
          .image-container(v-if="column === 'imageSrc'")
            img( v-for="(images) in item[column]" :src="'/src/assets/Untitled.png'" :alt="images.alt" style="width: 45%; max-width: 8rem;")
...
export default {
  ...
  setup(props) {
    ...
    const sortedItems = computed(() => {
      ...
      itemsCopy.sort((a, b) => {
        const aValue =
          typeof a[sortItemAttribute] === 'string'
            ? a[sortItemAttribute].toLowerCase().replace('/', '-')
            : a[sortItemAttribute]
        const bValue =
          typeof b[sortItemAttribute] === 'string'
            ? b[sortItemAttribute].toLowerCase().replace('/', '-')
            : b[sortItemAttribute]
        if (sortItemAttribute === 'releaseMonth') {
          return (months.indexOf(aValue) - months.indexOf(bValue)) * sortOrder.value
        }
        return typeof aValue === 'string'
          ? aValue.localeCompare(bValue) * sortOrder.value
          : (aValue - bValue) * sortOrder.value
      })
      return itemsCopy
    })
